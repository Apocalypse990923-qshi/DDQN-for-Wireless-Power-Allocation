...
#            return 14
    ...
    def remove_data(self):
        before_datas = os.listdir(Tools.duel_qf_data)
        if len(before_datas) != 0:
            for data in before_datas:
                os.remove(Tools.duel_qf_data + '/' + data)
        ...
        if len(before_datas) > 6:  # 保留最新的1份数据
            ...
            for i in range(len(before_datas)):
                ...
                if de_dat is not None:
                    os.remove(Tools.duel_qf_data + '/' + de_dat.group())
                    # print('已经删除数据：', de_dat.group())
        try:
            datas = shelve.open(Tools.duel_qf_data + '/' + filename, writeback=True, flag='c')
    ...
    def save_qlmodel(self, ql_model, ql_model_path):
        ...
        try:
            ...
            datas['duel_ql_model'] = ql_model
        except KeyError:
            print('duel_ql_model saving failed')
        finally:
            # print(filename + ',数据保存成功，数据大小：', epoch_data.shape)
            datas.close()
    ...
    def load_qlmodel(self, ql_model_path):
        ...
        for i in range(len(ql_models)):
            tim.append(int(ql_models[i].split('_')[2]))
        try:
            datas = shelve.open(ql_model_path + '/duel_qlmodel_' + str(np.max(tim)) + '_.db', writeback=True, flag='c')
            ql_model = datas['duel_ql_model']
            print('duel_ql_model loading success')
            return ql_model
        except KeyError:
            print('duel_ql_model loading fail')
            return None
        finally:
            # print(filename + ',数据保存成功，数据大小：', epoch_data.shape)
            datas.close()
    ...
    def start(self, model_num, epoch=100, iteration=400, only_q=True):
        ...
        self.net.start()
        if only_q:
            ql_model = self.load_qlmodel(Tools.duel_ql_model)
        else:
            ql_model = self.load_qlmodel(Tools.duel_ql_model_pro)
        ...
        flag = 0
        best_model = self.duel_qfnet.load_best_model()
        while self.count < epoch:
            outs = np.zeros((1, self.duel_qfnet.layers_units[3]))
            if not (only_q) and (best_model is not None):
                print('Enhanced Q-learing')
                epoch_data = Tools.get_epoch_data(self.net)
                outs = self.duel_qfnet.predict(best_model, epoch_data)
            iter = 0
            ...
            while iter < iteration:
                ...
                if only_q:
                    with open(Tools.qfnet_data + '/duel_' + model_num + '_only_c.txt', 'a') as only:
                        only.write(str(outs_capacity) + ',')
                else:
                    '''保存标签'''
                    with open(Tools.qfnet_data + '/duel_' + model_num + '_dqn_c.txt', 'a') as only:
                        only.write(str(outs_capacity) + ',')
            ...
            self.net.update()
        if only_q:
            self.save_qlmodel(self.Q, Tools.duel_ql_model)
        else:
            self.save_qlmodel(self.Q, Tools.duel_ql_model_pro)
