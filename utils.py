...
class Tools:
    ...
    duel_qf_data = qfnet_data + '/duel_qf_data'
    duel_qf_image = qfnet_data + '/duel_qf_image'
    duel_qf_model = qfnet_data + '/duel_qf_model'
    duel_ql_model_pro = qfnet_data + '/duel_ql_model_pro'
    duel_ql_model = qfnet_data + '/duel_ql_model'
    results = qfnet_data + '/results'
    ...
    def create_dirs(cls):
        ...
        if not (os.path.exists(cls.duel_qf_data)):
            os.mkdir(cls.duel_qf_data)
        if not (os.path.exists(cls.duel_qf_image)):
            os.mkdir(cls.duel_qf_image)
        if not (os.path.exists(cls.duel_qf_model)):
            os.mkdir(cls.duel_qf_model)
        if not (os.path.exists(cls.duel_ql_model_pro)):
            os.mkdir(cls.duel_ql_model_pro)
        if not (os.path.exists(cls.duel_ql_model)):
            os.mkdir(cls.duel_ql_model)
        if not (os.path.exists(cls.results)):
            os.mkdir(cls.results)
